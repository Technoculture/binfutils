...
class CSVLoader(DataLoader):
    ...
    # method to load data.fasta and file.txt file as input and return a dictionary object
    def load_data(filepath, file_name):

        # checks if file_name i.e. the txt file has already been created from the .fasta file to avoid large re-extraction
        if os.path.isfile(file_name):
            click.echo(f"{file_name} file already exists:)")
            pass
        else:
            # if provided file_name doesn't exists, create it. Extraction takes more than 2 hours.
        ...
        # a dictionary object is being created to be returned to avoid re-extraction
    ...
    def write_csv(obj_dict, csv_file):
        ...
        # checks if csv_file already exists
        if os.path.isfile(csv_file):

            click.echo(f"your {csv_file} csv file exists:)")
            pass
        else:
            # if csv_file doesn't exists takes a keyword prompt to determine which of the two csv file types are needed to be created
            ...
            # inner function to process dictionary object obj_dict to create sequences lengthwise occurence data for lengthwise_occurrence type csv
            ...
            # inner function to process dictionary object obj_dict to create sequences wise occurence data for sequence_occurrence csv
            ...
            # creating csv file type based on inputs
            ...
            # creating percentages of sequence_occurrences to write in sequences
            if type_csv == "sequence_occurrence":
    ...
    # method to generate and save histogram from lengthwise data csv input
    ...
    # method to generate and save certain number of maximum sequence occurence showing barh plot from sequence wise data csv
    ...
    # method to generate and save certain number of maximum sequence occurence showing pie-chart plot from sequence wise data csv
...
# the click cli commands function
...
@click.option("-t", "--type", "command", type=click.Choice(["fasta", "csv", "histo", "barh", 'pie'], case_sensitive=False), default="fasta", show_default=True, help="Type of operation")
